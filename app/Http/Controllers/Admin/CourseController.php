...
class CourseController extends Controller
{
    ...
    public function create()
    {
        ...
        $trainerName = Auth::user()->name; // Get the logged-in user's name
        return view('admin.courses_create', compact('packages', 'trainerName'));
    }
    ...
    {
        try {
            $course = Course::with(['reviews.user', 'enrollments'])->findOrFail($id);
            $enrolledCount = $course->enrollments->count();
            return view('courses.show', compact('course', 'enrolledCount'));
        } catch (\Exception $e) {
    ...
    public function edit($id)
    {
        try {
            $course = Course::with(['modules.videos', 'modules.documents', 'modules.quizzes'])
                ->findOrFail($id);
            $packages = Package::all();
            return view('admin.courses_edit', compact('course', 'packages'));
        } catch (\Exception $e) {
            return redirect()->route('admin.courses.index')->with('error', 'Course not found.');
        }
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('admin.courses.index')->with('error', 'Error deleting course: ' . $e->getMessage());
        }
    }
}
