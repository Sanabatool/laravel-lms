...
.course-header .video-preview {
  flex: 2;
  border-radius: 8px;
  overflow: hidden;
}

.course-header .video-preview img {
  width: 100%;
  height: 400px;
  object-fit: cover;
  border-radius: 8px;
...
<div id="reviews" class="tab-content">
  ...
  @auth
      <form method="POST" action="{{ route('courses.review', $course->id) }}">
          @csrf
  ...
  @if($reviews->count())
      @foreach($reviews as $review)
          <div class="card mb-3">
              <div class="card-body">
                  <h5 class="card-title">{{ $review->user->email ?? 'Unknown User' }}</h5>
                  <p class="card-text">
                      @for ($i = 1; $i <= 5; $i++)
                          @if ($i <= $review->rating)
  ...
  @else
      <div class="alert alert-warning">
          ...
          @auth
              <a href="#review-form">Be the first to review!</a>
          @endauth
      </div>
  @endif
</div> <!-- ✅ close reviews tab properly -->
